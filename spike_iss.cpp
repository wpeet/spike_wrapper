...
void spike_iss::help(int exit_code = 1)
{
...
void spike_iss::suggest_help()
{
  fprintf(stderr, "Try 'spike --help' for more information.\n");
  exit(1);
}

bool spike_iss::check_file_exists(const char *fileName)
{
  std::ifstream infile(fileName);
  return infile.good();
}

std::ifstream::pos_type spike_iss::get_file_size(const char *filename)
{
  std::ifstream in(filename, std::ios::ate | std::ios::binary);
  return in.tellg();
}

void spike_iss::read_file_bytes(const char *filename,size_t fileoff,
...
bool spike_iss::sort_mem_region(const mem_cfg_t &a, const mem_cfg_t &b)
{
...
bool spike_iss::check_mem_overlap(const mem_cfg_t& L, const mem_cfg_t& R)
{
  return std::max(L.get_base(), R.get_base()) <= std::min(L.get_inclusive_end(), R.get_inclusive_end());
}

bool spike_iss::check_if_merge_covers_64bit_space(const mem_cfg_t& L,
                                              const mem_cfg_t& R)
{
...
mem_cfg_t spike_iss::merge_mem_regions(const mem_cfg_t& L, const mem_cfg_t& R)
{
...
spike_iss::merge_overlapping_memory_regions(std::vector<mem_cfg_t> mems)
{
  if (mems.empty())
    return {};

  // FIXME: std::sort(mems.begin(), mems.end(), sort_mem_region);
...
mem_cfg_t spike_iss::create_mem_region(unsigned long long base, unsigned long long size)
{
...
std::vector<mem_cfg_t> spike_iss::parse_mem_layout(const char* arg)
{
...
std::vector<std::pair<reg_t, abstract_mem_t*>> spike_iss::make_mems(const std::vector<mem_cfg_t> &layout)
{
...
unsigned long spike_iss::atoul_safe(const char* s)
{
  char* e;
...
unsigned long spike_iss::atoul_nonzero_safe(const char* s)
{
...
std::vector<size_t> spike_iss::parse_hartids(const char *s)
{
...
spike_iss::spike_iss( sc_module_name name )
    : sc_module(name) 
{

  SC_THREAD(simulation_worker);
  sensitive << clk.pos();

  bridge = new SpikeSystemCBridge( );

}

void spike_iss::simulation_worker() {
  state_t* currentState;

  while (true) {
    // Wait for the next positive clock edge before moving forward
    wait();
    if( rst.read() == 1 ) { 
      cout << "spike_iss: Calling bridge->proc_ptr->reset() ..." << endl;
      bridge->proc_ptr->reset();
      currentState = bridge->proc_ptr->get_state();
      cout << "spike_iss: pc = " << hex << currentState->pc << endl;
    }
    else {
      // Intercept and redirect memory operations if necessary before stepping
      // Here, we explicitly instruct the processor instance to execute exactly one instruction
      // Parameter 'true' logs the step if interactive debugging hooks are set up
      // FIXME: cout << "spike_iss: Calling cpu->step(1) ..." << endl;
      //bridge->proc_ptr->step(1);
      currentState = bridge->proc_ptr->get_state();
      cout << "spike_iss: pc = " << hex << currentState->pc << endl;
    }
  }
...
void spike_iss::before_end_of_elaboration( ) {
  ...
  for ( int i = 0; i < argc; i++ ) {
    cout << "spike_iss: argv[" << i << "] = " << argv[i] << endl;
  }
  ...
  for( int i = 0; i < (argc-1); i++ ) {
    cout << "spike_iss: argv1[" << i << "] = " << argv1[i] << endl;
  }
  for( int i = 0; i < htif_args.size(); i++ ) {
    cout << "spike_iss: htif_args[" << i << "] = " << htif_args[i] << endl;
  }

  cout << "spike_iss: log = " << log << endl;
  cout << "spike_iss: cfg.isa = " << cfg.isa << endl;

  auto start_pc = cfg.start_pc.get(0);
  if (start_pc.has_value()) {
    cout << "spike_iss: start_pc = " << hex << start_pc.value() << endl;
  }
  else {
    cout << "spike_iss: start_pc does not have a value." << endl;
  }

  mem_cfg_t* memCfg;
  for( int i = 0; i < cfg.mem_layout.size(); i++ ) {
    memCfg = &cfg.mem_layout[i];
    cout << "spike_iss: memCfg[" << i << "].base = " << hex << memCfg->get_base() << endl;
    cout << "spike_iss: memCfg[" << i << "].size = " << hex << memCfg->get_size() << endl;
  }
  ...
  spike_sim = new sim_t(  &cfg, halted,
                          mems, plugin_device_factories, dtb_discovery, 
  ...
  std::unique_ptr<jtag_dtm_t> jtag_dtm(
      new jtag_dtm_t(&spike_sim->debug_module, dmi_rti));
  if (use_rbb) {
    remote_bitbang.reset(new remote_bitbang_t(rbb_port, &(*jtag_dtm)));
    spike_sim->set_remote_bitbang(&(*remote_bitbang));
  }
  ...
  // FIXME: auto return_code = s.run();

  for (auto& mem : mems)
    delete mem.second;

  // FIXME: return return_code;

} // before_end_of_elaboration

void spike_iss::end_of_elaboration( ) {

  cout << "spike_iss: Getting pointer to cpu ..." << endl;
  bridge->proc_ptr = spike_sim->get_core(0);
  cout << "spike_iss: Getting pointer to cpu successful!" << endl;
